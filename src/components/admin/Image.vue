      ...
      <v-container grid-list-md text-xs-center>
        ...
        >
          ...
          <v-dialog v-model="dialog" max-width="90vw" persistent>
            <add-carousel-image></add-carousel-image>
            <v-fab-transition>
              <v-btn 
                fab
                small
...
import AddCarouselImage from "@/components/oscomponents/form/AddCarouselImage"
import * as firebase from "firebase";

export default {
    components: { AdminToolbar, AddCarouselImage },  
    data: () => ({
      dialog: false,
      show: false,
			toggle: [],
    ...
    created () {
      firebase.auth().onAuthStateChanged((user) => {
        if (user) {
          this.$store.dispatch("loadCarouselImage");
        }
      })
    },
    computed: {
    ...
    methods: {
      deleteImage (a, b) {
        ...
        function imageToDelete () {
          local.$store.dispatch("deleteCarouselImage", imageData)
        }
